    ...
    if [ -d "$input_folder" ]; then
        ...
        for input_file in "$input_folder"/* ; do
            ...
            if [[ "$base_name" =~ -cli-([0-9]+\.[0-9]+\.[0-9]+)- ]]; then
                ...
                # Create the releasenote.md file with the specified content
                echo "## $version ($(date +%m/%d/%Y))" > "ipm-releases/$version/releasenote.md"
                echo "" >> "ipm-releases/$version/releasenote.md"
                echo "#### IPM $version Release Notes" >> "ipm-releases/$version/releasenote.md"
                echo "**New features**:" >> "ipm-releases/$version/releasenote.md"
                echo "**Enhancements**:" >> "ipm-releases/$version/releasenote.md"
                echo "**Bug fixes**:" >> "ipm-releases/$version/releasenote.md"
                echo "----" >> "ipm-releases/$version/releasenote.md"
            else
...
for file in "ipm-releases/$version"/* ; do
    if [[ "$(basename "$file")" != *.sha256 && "$(basename "$file")" != *.md ]]; then
        ...
        echo "* $(basename "$file")" >> "ipm-releases/$version/releasenote.md"
        echo "    * $file_hash" >> "ipm-releases/$version/releasenote.md"
    fi
done
